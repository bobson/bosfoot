...
function buildBrandDoc(b: CatalogBrand): SanityDoc {
  return {
    ...
    description: b.description
      ? { _type: "localeText", ...b.description }
      : undefined,
    sizeChart: b.sizeChart
      ? {
          ...
          notes: b.sizeChart.notes
            ? { _type: "localeText", ...b.sizeChart.notes }
            : undefined,
        }
      : undefined,
...
function buildCategoryDoc(c: CatalogCategory): SanityDoc {
  return {
    ...
    slug: {
      ...
      sq: c.name.sq
        ? { _type: "slug", current: asciifySlug(c.name.sq) }
        : undefined,
      en: c.name.en
        ? { _type: "slug", current: asciifySlug(c.name.en) }
        : undefined,
    },
    description: c.description
      ? { _type: "localeText", ...c.description }
      : undefined,
    order: c.order ?? 100,
    parent: c.parentSlug
      ? { _type: "reference", _ref: ids.category(c.parentSlug) }
      : undefined,
  };
}
...
function buildProductDoc(p: CatalogProduct): SanityDoc {
  return {
    ...
    slug: {
      ...
      sq: p.name.sq
        ? { _type: "slug", current: asciifySlug(p.name.sq) }
        : undefined,
      en: p.name.en
        ? { _type: "slug", current: asciifySlug(p.name.en) }
        : undefined,
    },
    ...
    specs: p.specs
      ? {
          ...
          soleMaterial: p.specs.soleMaterial
            ...
            : undefined,
          lining: p.specs.lining
            ? { _type: "localeString", ...p.specs.lining }
            : undefined,
...
function validate(catalog: Catalog): string[] {
  ...
  for (const c of catalog.categories) {
    ...
    if (c.parentSlug && !categorySlugs.has(c.parentSlug)) {
      errors.push(
        `Category ${c.slug} references unknown parent: ${c.parentSlug}`,
      );
    }
  }
...
async function buildPlan(catalog: Catalog): Promise<PlanItem[]> {
  ...
  const existingArr = await client.fetch<SanityDoc[]>(`*[_id in $ids]`, {
    ids: allIds,
  });
  ...
  for (const b of catalog.brands) {
    ...
    plan.push(
      ...diffOrCreate("brand", b.name, newDoc, existing.get(newDoc._id)),
    );
  }
...
function stripInternal(obj: unknown): unknown {
  ...
  if (obj && typeof obj === "object") {
    const out: Record<string, unknown> = {};
    for (const [k, v] of Object.entries(obj)) {
      if (k.startsWith("_") && k !== "_type" && k !== "_ref" && k !== "_key")
        continue;
...
function printPlan(plan: PlanItem[]) {
  ...
  console.log(
    c.dim +
      `Project: ${sanityProjectInfo.projectId} / ${sanityProjectInfo.dataset}` +
      c.reset,
  );
  console.log();
  ...
  if (missingImages > 0) {
    console.log(
      `  ${c.yellow}Products without main image: ${missingImages}${c.reset}`,
    );
  }
  console.log();
  ...
  switch (item.kind) {
    ...
    case "update":
      ...
      if (item.diffLines.length > 8) {
        console.log(
          c.dim +
            `    ... and ${item.diffLines.length - 8} more changes` +
            c.reset,
        );
      }
      break;
...
async function applyPlan(plan: PlanItem[]) {
  ...
  type ImageMap = {
    main?: { _ref: string };
    gallery: Array<{ _key: string; _ref: string }>;
  };
  ...
  console.log(
    c.dim +
      `View at https://${sanityProjectInfo.projectId}.sanity.studio` +
      c.reset,
  );
  console.log(
    c.dim + "or http://localhost:3333 if studio is running." + c.reset,
  );
  console.log();
}
...
async function main() {
  ...
  try {
    ...
    console.error(
      c.red + "Failed to read catalog.json:" + c.reset,
